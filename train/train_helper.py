...
def draw_someoutput(matrices,savefold,chosen_cmap='viridis',draw_suffix='some_mats',no_axis=False):
            ...
            if ind%batch_size ==batch_size -1:
                ...
                if do_single_patch:
                    ...
                    if two_patch_instance:
                        ...
                        yield np.concatenate((xmil,xax,xback),axis=-1),y
                elif do_key:
...
def save_relevant(savedir,add_str,files=None,str_to_save=None,just_return=False,descriptive_text=None):
    ...
    '''
    if files is None:
        files='net_models.py;dnn_workspace.py'.split(';')
...
def use_best_model(shrt_str):
    starting=r'D:\code_projects\Neuron_fibers_workspace\models\replication'
    models=[]
...
def draw_autoenc_smaller(model,draw_loc,imgs):
    ...
    for ii in range(6):
        cv2.imwrite(os.path.join(draw_loc, 'ae_{}_input.tif'.format(ii))
                    , (merge_graychannels(imgs[0,ii,1:-2,1:-2]))*127)
        cv2.imwrite(os.path.join(draw_loc, 'ae_{}_output.tif'.format(ii))
                    , (   merge_graychannels(imgs[1,ii,1:-2,1:-2]))*127)
        cv2.imwrite(os.path.join(draw_loc, 'ae_{}_autoenc_recreation.tif'.format(ii))
                    , rec_x[ii,1:-2,1:-2]*127)

        for jj in range(40):
            cv2.imwrite(os.path.join(draw_loc, 'ae_{}_dnnInput_{}.tif'.format(ii,jj))
                        , ((enc_x[ii,:,:,jj]/np.max(enc_x[ii,:,:,jj]))*255).astype(np.uint8) )



def draw_autoenc(model,img_w,img_h,draw_loc,val_loc):
    ...
    draw_someoutput(merge_graychannels(base_i[:,0,:,:,:])
                    ,draw_loc,draw_suffix='enc_inputs',
                    chosen_cmap='gray')

    draw_someoutput(merge_graychannels(base_i[:, 1, :, :, :]),
                    draw_loc, draw_suffix='enc_annotation',
    chosen_cmap = 'gray')

    enc_x = encD([base_i[:,0,:,:,:], 0])[0]
    rec_x=decD([ base_i[:,0,:,:,:] ,0])[0]
    pass
    draw_someoutput(merge_graychannels(rec_x),draw_loc,draw_suffix='enc_recreations',chosen_cmap='gray')
    for indP,enc_xx in enumerate(enc_x):
        enc_matx=np.transpose(enc_xx,(2,0,1))
        draw_someoutput(enc_matx, draw_loc,draw_suffix='rec_mars_{}'.format(indP), chosen_cmap='gray')
...
def quick_test(model, addition_str, big_image_size, do_all=False, draw_img_ext='*.tiff', test_batch_size=180,
    ...
    from test_predictions_funcs import create_prediction, \
    ...
    output_prediction_file = r"D:\data_projects\neuron_fibers_data\predictions/quick_test_{}.txt".format(addition_str)

    colored_images = r'D:\data_projects\neuron_fibers_data\images\predicted/quick_test_{}/'.format(addition_str)
    ...
    color_images_in_folder( colored_images,output_prediction_file,int(image_width/2),
                            big_image_size,*colors_grey,suf_img='-gray_predictions-{}.tif'.format(addition_str)
        )
    ...
    color_mistakes_in_folder(  colored_images,groundtruth_path,
                               ...
                               suf_img='-colored_train_mistakes-{}.tif'.format(addition_str),
...
if __name__=='__main__':
    train_lmdb = r'D:\NEURON_FIBER_DATA\lmdb_base_interim_2k_45w_roi_cells_unifw_noise_test_nexp_mask\train_db'
