...
def get_2k_image_pretrained(img_w,img_h,added_stuff=None,ch_add=3):
    ...
    if added_stuff['use_crbm']:
        ...
        model.add(BatchNormalization(input_shape=(img_w+3, img_h+3, ch_add)))
...
def get_autoencoded(imgw,imgh):
    input_img = Input(shape=(imgw+3, imgh+3, 3))  # adapt this if using `channels_first` image data format
