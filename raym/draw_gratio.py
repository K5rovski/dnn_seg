...
def main_loop(confD):
    ...
    if caseType2 is not None:
    	...
    	JUSTSIMPLE=[not (row.isUnbalanced | row.isB) for ind,row in dnnI.iterrows() if row.goodStopC==False]
    	plt.scatter(cor[:,1],cor[:,0],facecolors='red', edgecolors='none',label='ROIs No_{0} = {1}'.format(caseType2,cor.shape[0]))
    	plt.scatter(dnn[JUSTSIMPLE,1],dnn[JUSTSIMPLE,0],
    	facecolors='none', edgecolors='green',label='ROIs No_{0} = {1}'.format(caseType1, np.sum(JUSTSIMPLE) ))#afterc_str if dotwoCases else total_str ))
    else:
    	mapD=np.ones((dnn.shape[0],))*100
    	plt.scatter(dnn[JUSTSIMPLE,1],dnn[JUSTSIMPLE,0],facecolors='none', edgecolors='green',label='{} Rois'.format(caseType1))
    ...
    regrtdnnG=fit_reg(dnn[JUSTSIMPLE,1].reshape(-1, 1), dnn[JUSTSIMPLE,0].reshape(-1, 1))
    ...
    # if caseType2 is not None: print('corrected coun: ',np.sum(matchCM),np.sum(JUSTSIMPLE))
    ...
    meanDnn=np.mean(dnn[JUSTSIMPLE,0])
    plt.plot([0,120],np.repeat(np.mean(dnn[JUSTSIMPLE,0]),2)
    		,color='green',ls='--',
    ...
    if caseType2 is not None:
    	...
    	plt.text(5,0.87,'|Mean_{1} - Mean_{0}| / Mean_{0} = {2:.2f}%'.format(caseType2,caseType1,100*abs(meanCor-meanDnn)/meanCor ), \
    			 fontdict={'size':25}, bbox=dict(facecolor='white', edgecolor='none'))
    	# plt.text(22,0.04,'Rois: {}={},{}={}'.format(caseType1,np.sum(JUSTSIMPLE),caseType2,cor.shape[0]), \
    			 # fontdict={'size':35})
    ...
    title_text='Total {0} ROIs= {2},  Measured {0} ROIs= {3},  No.{1} ROIs={4}'.format(caseType1,caseType2,np.sum(mapD>0),np.sum(JUSTSIMPLE),cor.shape[0])
    title_text=''
    ...
    plt.figure(figsize=(20,20))
    # plt.subplot(211)
    plt.title(confD.get('hist_title',''))
    plt.xlabel('Minor size (px)')
    plt.ylabel('Frequency')

    plt.hist(dnn[JUSTSIMPLE,1],range=[0,100],bins=200,alpha=0.8,ec='green',lw=3,fc='none',label='{} hist, {} ROIs.'.format(caseType1,np.sum(JUSTSIMPLE) ))
